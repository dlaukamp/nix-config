...
nix-shell -p git --command "git clone git@github.com:dlaukamp/nix-config.git $SCRIPT_DIR"
...
sed -i "s/david@lauka.mp//" $SCRIPT_DIR/flake.nix
...
nix run home-manager/master --extra-experimental-features nix-command --extra-experimental-features flakes -- switch --flake $SCRIPT_DIR#dlkmp;
